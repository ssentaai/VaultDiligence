...
**Sub-Category**: Service Provider
**Field Name**: Crypto Custodian — Named Party
**What to Collect / Question to Answer**: Name the party that holds the on-chain / crypto assets, where distinct from the securities custodian in this vault's stack. If the role exists in the structure but is undisclosed, record it as gapped; if no such role exists in this structure, record N/A with the reason.
...
**Institutional Standard**: The party filling this role is named to a verifiable entity, or the role is explicitly recorded as gapped or N/A — never a silent absence. Detail: F-ENT-045 (MPC / Execution Wallet Type).
...
**Source / Precedent**: role-registry-reconciliation 2026-07-08: only partially covered by F-ENT-045 (wallet type); the crypto-custodian party (e.g. Circle, Fireblocks) was not separately named.
**Criterion ID(s)**: 17.2 (P17 Counterparty & Role Registry)
